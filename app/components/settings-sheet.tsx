...
type PageId = "root" | "diagnostics" | "profile" | "general" | "privacy" | "capabilities" | "voice";
...
function SectionHeader({ children }: { children: ReactNode }) {
  return <h3 className="mb-2 mt-8 px-4 text-[0.875rem]/5 font-semibold text-secondary first:mt-4 uppercase tracking-wide">{children}</h3>;
}
...
function Row({ label, description, control, fullWidthControl }: {
  ...
  return (
    <div className="px-4 py-3.5 bg-app active:bg-elev-2 transition-colors">
      <div className="flex items-center gap-3">
        <span className="min-w-0 flex-1 text-[1rem]/[1.375rem] font-medium text-primary">{label}</span>
        {control}
      </div>
      {description ? <p className="mt-1 max-w-[92%] text-[0.8125rem]/[1.125rem] text-tertiary">{description}</p> : null}
...
function Group({ children }: { children: ReactNode }) {
  return <div className="divide-y divide-[var(--border-subtle)] border-y border-[var(--border-subtle)] bg-app my-2">{children}</div>;
}
...
export function SettingsToggle({ value, onChange, label }: { value: boolean; onChange: () => void; label: string }) {
  return (
    <button
      ...
      aria-label={label}
      onClick={onChange}
      className={`relative h-[30px] w-[50px] shrink-0 rounded-full transition-colors duration-[120ms] ${value ? "bg-[#0A84FF]" : "bg-elev-3"}`}
    >
      <span className={`absolute top-[3px] h-6 w-6 rounded-full shadow-sm transition-transform duration-[140ms] ${value ? "translate-x-[21px] bg-white" : "translate-x-[3px] bg-white"}`} />
    </button>
  );
}
...
function BareSelect({ value, options, onChange, label }: {
  ...
  return (
    <span className="relative flex shrink-0 items-center gap-1.5 text-[1rem]/5 text-secondary">
      {current}
      <ChevronRight size={16} className="rotate-90 text-tertiary" />
      <select
        aria-label={label}
        value={value}
...
function TextField({ value, onChange, label, placeholder }: {
  ...
  return (
    <input
      ...
      onChange={(event) => onChange(event.target.value)}
      className="h-10 w-[55%] min-w-0 shrink-0 rounded-[10px] bg-elev-2 px-3 text-right text-[0.9375rem]/5 text-primary outline-none placeholder:text-tertiary focus:bg-elev-3"
    />
  );
}
...
function InlineButton({ children, onClick, destructive }: { children: ReactNode; onClick: () => void; destructive?: boolean }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`h-9 shrink-0 rounded-full bg-elev-2 px-4 text-[0.875rem]/5 font-semibold active:bg-elev-3 ${destructive ? "text-danger" : "text-primary"}`}
    >
      {children}
    </button>
  );
...
function RootRow({ label, active, onOpen, icon }: { label: string; active?: boolean; onOpen: () => void; icon?: ReactNode }) {
  return (
    <button
      ...
      className={`flex min-h-[52px] w-full items-center justify-between px-4 text-left active:bg-elev-2 bg-app ${active ? "md:bg-elev-2" : ""}`}
    >
      <div className="flex items-center gap-3">
        {icon}
        <span className="text-[1rem]/[1.375rem] font-medium text-primary">{label}</span>
      </div>
...
function ThemeCard({ theme, active, onClick, label }: { theme: "light" | "dark" | "system", active: boolean, onClick: () => void, label: string }) {
  return (
    <button type="button" onClick={onClick} className={`flex flex-col items-center gap-2 p-2.5 rounded-[18px] bg-elev-2 active:scale-95 transition-all ${active ? "ring-2 ring-[#0A84FF]" : "ring-1 ring-[var(--border-subtle)]"}`}>
      <div className="w-[60px] h-[42px] rounded-[8px] overflow-hidden flex shadow-sm border border-black/10 dark:border-white/10 relative">
         {theme === "light" && <div className="w-full h-full bg-[#FFFFFF]" />}
         {theme === "dark" && <div className="w-full h-full bg-[#1C1C1E]" />}
         {theme === "system" && (
            <>
         ...
         <div className={`absolute top-1.5 left-1.5 w-4 h-0.5 rounded-full ${theme === "light" || theme === "system" ? "bg-black/20" : "bg-white/20"}`} />
         <div className="absolute bottom-1.5 right-1.5 w-2.5 h-2.5 rounded-full bg-[#FF453A]" />
      </div>
      <span className={`text-[0.8125rem] font-medium text-primary`}>{label}</span>
    </button>
  )
}
...
export function SettingsSheet({
  ...
  const openPage = (next: PageId) => {
    ...
    setPage(next);
    if (next !== "root" && next !== "diagnostics" && next !== "profile" && next !== "voice") {
    ...
    <div className="fixed inset-0 z-[95] flex flex-col bg-[#F2F2F7] dark:bg-black" role="dialog" aria-modal="true" aria-label="Settings">
      <header className="navi-sheet-header sticky top-0 z-10 flex h-[calc(52px+var(--safe-top))] shrink-0 items-center gap-1 bg-[#F2F2F7] dark:bg-black px-2 pt-[var(--safe-top)]">
        {page === "root" ? (
          ...
          <button type="button" onClick={() => setPage("root")} aria-label="Back to Settings" className="flex h-11 w-11 items-center justify-center rounded-full text-accent active:opacity-60 md:hidden">
        ...
        <button type="button" onClick={onClose} aria-label="Close settings" className="flex h-11 w-[72px] items-center justify-end pr-3 rounded-full text-accent font-semibold active:opacity-60">
          Done
        </button>
      </header>
      ...
      <div className="flex min-h-0 flex-1 md:mx-auto md:w-full md:max-w-[1000px]">
        ...
        >
          <div className="mt-4 mb-2 px-4 md:hidden">
             <h2 className="text-[2rem]/8 font-bold text-primary">Settings</h2>
          </div>

          <div className="mt-4">
            <Group>
              <button type="button" onClick={() => openPage("profile")} className="flex min-h-[76px] w-full items-center justify-between px-4 text-left active:bg-elev-2 bg-app transition-colors">
                 <div className="flex items-center gap-4">
                   <div className="w-[52px] h-[52px] rounded-full bg-gradient-to-br from-gray-300 to-gray-400 flex items-center justify-center text-[1.375rem] font-medium text-black shadow-sm">
                     {profileInitial}
                   </div>
                   <div className="flex flex-col">
                     <span className="text-[1.0625rem]/6 font-medium text-primary">
                       {preferences.profile.displayName || preferences.profile.fullName || account.email || "Profile"}
                     </span>
                     <span className="text-[0.8125rem]/5 text-tertiary">Account, Email, Personal info</span>
                   </div>
                 </div>
                 <ChevronRight size={20} className="text-tertiary" />
              </button>
          ...
          <div className="mt-6">
            <Group>
              <RootRow label="General" active={page === "general"} onOpen={() => openPage("general")} icon={<Settings size={20} className="text-secondary" />} />
              <RootRow label="Connectors" onOpen={() => { onClose(); onOpenConnectors(); }} icon={<Link2 size={20} className="text-secondary" />} />
              <RootRow label="Capabilities" active={page === "capabilities"} onOpen={() => openPage("capabilities")} icon={<FlaskConical size={20} className="text-secondary" />} />
            </Group>
          </div>

          <div className="mt-6">
            <Group>
              <RootRow label="Memory and storage" active={page === "privacy"} onOpen={() => openPage("privacy")} icon={<Shield size={20} className="text-secondary" />} />
            </Group>
          </div>
          ...
          <button
            ...
            className="w-full mt-8 px-4 py-6 text-center text-[0.75rem]/4 text-tertiary"
        ...
        <div className={`min-h-0 flex-1 overflow-y-auto overscroll-contain pb-[calc(24px+var(--safe-bottom))] ${page === "root" ? "hidden md:block" : ""}`}>
          ...
          {page === "profile" ? (
            <div className="pb-10 pt-2">
              ...
              <Group>
                ...
                <div className="px-4 py-1.5 text-[0.75rem]/[1.125rem] text-tertiary bg-[#F2F2F7] dark:bg-black">
              ...
              {account.signedIn && (
                <div className="mt-8 px-4">
                  <button onClick={() => void signOut()} className="flex items-center justify-center gap-2 w-full h-[44px] rounded-xl bg-elev-2 text-danger font-medium text-[0.9375rem] active:bg-elev-3">
          ...
          {page === "general" ? (
            <div className="pb-10 pt-2">
              ...
              <div className="px-4 py-4 flex gap-4 justify-center bg-app border-y border-[var(--border-subtle)]">
              ...
              <Group>
                <RootRow label="Permissions" onOpen={() => {}} icon={<Key size={20} className="text-secondary" />} />
                <RootRow label="Voice" onOpen={() => setPage("voice")} icon={<Volume2 size={20} className="text-secondary" />} />
                <div className="flex min-h-[52px] w-full items-center justify-between px-4 bg-app active:bg-elev-2 transition-colors">
                  <div className="flex items-center gap-3">
                    <Smartphone size={20} className="text-secondary" />
                    <span className="text-[1rem]/[1.375rem] font-medium text-primary">Haptic feedback</span>
                  </div>
              ...
              <Group>
                <button type="button" onClick={() => { haptic("impact-light", preferences.haptics); setUpdateStatus({ phase: "checking", message: "Checking for the newest version…" }); requestPwaUpdate(); }} disabled={updateBusy} className="flex w-full items-center justify-between px-4 py-3.5 text-left active:bg-elev-2 disabled:opacity-70 bg-app">
                  <span className="min-w-0 flex-1">
                    <span className="block text-[1rem]/[1.375rem] font-medium text-primary">Update NaviOS</span>
                    <span className="block text-[0.8125rem]/[1.125rem] text-secondary">{versionLabel()}</span>
                    <span className={`block text-[0.75rem]/4 mt-0.5 ${updateStatus.phase === "error" ? "text-danger" : "text-tertiary"}`}>{updateStatus.message}</span>
                  </span>
              ...
              <div className="mt-8 px-4">
                 <button onClick={() => { if (window.confirm("Clear all NaviOS history, projects, and settings from this device?")) { onClearData(); onClose(); } }} className="flex items-center justify-center gap-2 w-full h-[44px] rounded-xl bg-elev-2 text-danger font-medium text-[0.9375rem] active:bg-elev-3">
                   Clear Local Data
                 </button>
              </div>
            </div>
          ...
          {page === "capabilities" ? (
            <div className="pb-10 pt-2">
              ...
              <Group>
                <Row label="Web search" description="Search the web and read pages when a request needs live information." control={<SettingsToggle label="Web search" value={preferences.tools.web} onChange={() => update({ tools: { ...preferences.tools, web: !preferences.tools.web } })} />} />
                <Row label="Artifacts" description="Build interactive documents and designs in a window alongside the conversation." control={<SettingsToggle label="Artifacts" value={preferences.tools.artifacts} onChange={() => update({ tools: { ...preferences.tools, artifacts: !preferences.tools.artifacts } })} />} />
                <Row label="Code execution" description="Run JavaScript on this device to check its own work before answering, then fix what fails." control={<SettingsToggle label="Code execution" value={preferences.tools.code} onChange={() => update({ tools: { ...preferences.tools, code: !preferences.tools.code } })} />} />
              </Group>
                  ...
                  fullWidthControl={
                    <div>
                      ...
                      <textarea
                        ...
                        className="mt-2 min-h-[100px] w-full resize-y rounded-[12px] bg-elev-2 px-3.5 py-3 text-[0.9375rem]/[1.375rem] text-primary outline-none placeholder:text-tertiary focus:bg-elev-3"
                      />
                      <div className="mt-3 flex items-center gap-3">
                        <button
                          ...
                          className="h-9 rounded-full bg-accent px-4 text-[0.8125rem]/5 font-semibold text-[var(--accent-on-primary)] active:opacity-80 disabled:opacity-50"
                        >
                ...
                <div key={group.category}>
                  ...
                  <Group>
                    {group.skills.map((skill: Skill) => (
                      <div key={skill.id} className="px-4 py-3 bg-app">
                  ...
                  fullWidthControl={
                    <div>
                      <textarea
                        ...
                        rows={4}
                        className="min-h-[100px] w-full resize-y rounded-[12px] bg-elev-2 px-3.5 py-3 font-mono text-[0.8125rem]/[1.125rem] text-primary outline-none placeholder:text-tertiary focus:bg-elev-3"
                      />
                      <div className="mt-3 flex items-center gap-3">
                        <button
                          type="button"
          ...
          {page === "privacy" ? (
            <div className="pb-10 pt-2">
              ...
              <Group>
                ...
                <Row label="Memory" description="Let a new chat draw on relevant passages from your earlier ones." control={<SettingsToggle label="Memory" value={preferences.memory} onChange={() => update({ memory: !preferences.memory })} />} />
              </Group>
          ...
          {page === "diagnostics" ? (
            <div className="pb-10 pt-2">
              ...
              <Group>
                <button type="button" onClick={() => void runEvals()} disabled={evalState.phase === "running"} className="flex w-full items-center gap-3 px-4 py-3.5 text-left active:bg-elev-2 disabled:opacity-70 bg-app">
                  <span className="min-w-0 flex-1">
                    <span className="block text-[0.9375rem]/[1.375rem] font-medium text-primary">Run quality check</span>
                    <span className={`block text-[0.8125rem]/[1.125rem] ${evalState.phase === "error" ? "text-danger" : "text-secondary"}`}>{evalState.message}</span>
                  </span>
