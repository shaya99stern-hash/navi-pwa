    ...
    <div className="fixed inset-0 z-[120] flex flex-col bg-app text-primary">
      <header className="safe-top flex min-h-[64px] shrink-0 items-center gap-2 border-b border-[var(--border-subtle)] bg-elev-1 px-3">
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-[17px]/6 font-semibold text-primary">Connectors</span>
          <span className="block text-[11px]/4 font-medium text-tertiary">{connectedCount} connected · mobile access and approvals</span>
        </span>
        ...
        <div className="mx-auto w-full max-w-[760px] px-4 pb-[calc(24px+var(--safe-bottom))] pt-4">
          <section className="rounded-[24px] border border-[var(--border-subtle)] bg-elev-1 p-4">
            <div className="flex items-start gap-3">
              ...
              <span className="min-w-0 flex-1">
                <h2 className="text-[15px]/5 font-semibold text-primary">Conversation access mode</h2>
                <p className="mt-1 text-[11px]/4 font-medium text-tertiary">Controls how connected resources may be used. Sensitive external actions never bypass confirmation.</p>
              </span>
            </div>
            <div className="mt-4 space-y-2">
              {MODES.map((mode) => (
                <button key={mode.id} type="button" onClick={() => { update({ connectorAccessMode: mode.id }); haptic("selection", haptics); }} className={`flex min-h-[72px] w-full items-center gap-3 rounded-2xl border px-3 text-left ${preferences.connectorAccessMode === mode.id ? "border-accent bg-[var(--selection-bg)]" : "border-[var(--border-subtle)] bg-elev-2 active:bg-elev-3"}`}>
                  <span className="min-w-0 flex-1"><span className="block text-[14px]/5 font-semibold text-primary">{mode.title}</span><span className="block text-[11px]/4 font-medium text-tertiary">{mode.detail}</span></span>
          ...
          <section className="mt-4 rounded-[24px] border border-[var(--border-subtle)] bg-elev-1 p-4">
            <div className="flex items-start gap-3">
              ...
              <span className="min-w-0 flex-1"><h2 className="text-[15px]/5 font-semibold text-primary">Available connectors</h2><p className="mt-1 text-[11px]/4 font-medium text-tertiary">Configured through the server-side MCP registry. Credentials never enter the browser.</p></span>
            </div>

            {errors.directory ? <div className="mt-3 flex gap-2 rounded-2xl border border-[var(--accent-danger)] bg-elev-2 p-3 text-[12px]/4 font-medium text-danger"><AlertTriangle size={16} className="shrink-0" />{errors.directory}</div> : null}
                ...
                return (
                  <div key={server.id} className="py-3">
                    <div className="flex min-h-14 items-center gap-3">
                      ...
                      <span className="min-w-0 flex-1"><span className="block truncate text-[14px]/5 font-semibold text-primary">{server.name}</span><span className="block truncate text-[11px]/4 font-medium text-tertiary">Remote HTTPS MCP · {server.readOnly ? "read-only" : "writes require confirmation"}</span></span>
                    ...
                    {errors[server.id] ? <div className="ml-[52px] mt-1 flex gap-1.5 text-[11px]/4 font-medium text-danger"><AlertTriangle size={14} className="shrink-0" />{errors[server.id]}</div> : null}
                  </div>
                );
              })}
              {!loading && !servers.length ? <div className="py-10 text-center text-[13px]/5 font-medium text-tertiary">No connector servers are configured in Vercel.</div> : null}
            </div>
          </section>

          <section className="mt-4 rounded-[24px] border border-[var(--border-subtle)] bg-elev-1 p-4">
            <div className="flex gap-3"><LockKeyhole size={19} className="mt-0.5 shrink-0 text-accent" /><div><h2 className="text-[14px]/5 font-semibold text-primary">Approval contract</h2><p className="mt-1 text-[12px]/5 font-medium text-secondary">Reads may follow the selected access mode. Writes, purchases, bookings, deletes, and external sends require an explicit approval step. Disconnected, expired, blocked, and unsupported connectors remain visibly unavailable.</p></div></div>
          </section>
        </div>
      </main>
    </div>
