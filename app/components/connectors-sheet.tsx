                ...
                {catalog.providers.length ? catalog.providers.map((provider, index) => {
                  ...
                  return (
                    <div key={provider.id}>
                      ...
                      {expanded ? (
                        <div className="border-t border-[var(--border-subtle)] connector-soft-elev-30 px-4 py-3 pl-[58px]">
          ...
          <Group>
            ...
            {advancedOpen ? (
              <div className="border-t border-[var(--border-subtle)]">
                ...
                {mcpOpen ? (
                  <div className="border-y border-[var(--border-subtle)] connector-soft-elev-20">
                ...
                {customOpen ? (
                  <div className="border-t border-[var(--border-subtle)] connector-soft-elev-20">
          ...
          {apiOpen ? (
            <Group>
              <div className="space-y-2 p-3">
                ...
                {discovery.phase === "failed" ? <div className="rounded-[9px] connector-soft-danger p-2.5 text-[12px] text-danger"><div className="font-medium">{discovery.message}</div>{discovery.detail ? <div className="mt-1 break-all opacity-80">{discovery.detail}</div> : null}</div> : null}
                {discovery.phase === "found" ? <div className="rounded-[9px] border border-accent/20 bg-accent/5 p-3"><div className="text-[14px] font-semibold text-primary">{discovery.manifest.name}</div><div className="mt-1 text-[12px] text-tertiary">{discovery.summary.operations} operations · {discovery.summary.reads} reads{discovery.summary.writes ? ` · ${discovery.summary.writes} writes` : ""}</div>{discovery.summary.truncated ? <div className="mt-1 text-[12px] text-warning">This API describes {discovery.summary.truncated.declared} operations; Navi can keep the first {discovery.summary.truncated.kept} here.</div> : null}<button type="button" onClick={saveApi} className="mt-2 h-9 rounded-[9px] bg-accent px-4 text-[13px] font-semibold text-white"><Check size={14} className="mr-1 inline" />Add API</button></div> : <button type="button" onClick={() => void discoverApi()} disabled={discovery.phase === "looking" || !apiUrl.trim()} className="h-10 w-full rounded-[9px] bg-accent text-[13px] font-semibold text-white disabled:opacity-45">{discovery.phase === "looking" ? "Reading API…" : "Read API"}</button>}
              </div>
            </Group>
          ) : null}
