  ...
  return (
    <div className="flex min-h-[58px] items-center gap-3 px-4 py-2.5">
      <span className="min-w-0 flex-1">
        <span className="block text-[0.9375rem]/[1.375rem] font-medium text-primary">{title}</span>
        {detail ? <span className="block text-[0.75rem]/4 font-medium text-tertiary">{detail}</span> : null}
      </span>
      {action}
    </div>
  );
        ...
        <div className="fixed inset-0 z-[90]">
          ...
          <section {...sheet.sheetProps} className="navi-sheet absolute inset-x-0 bottom-0 mx-auto flex max-h-[86dvh] w-full max-w-[720px] flex-col overflow-hidden md:max-w-[480px]">
            ...
            <header className="flex h-12 shrink-0 items-center justify-between px-4">
              <div className="text-[1.0625rem]/6 font-semibold tracking-[-0.01em] text-primary">Options</div>
            ...
            <nav className="scroll-area flex shrink-0 gap-1 overflow-x-auto border-b border-[var(--border-subtle)] px-3 py-2" aria-label="NaviOS Hub menu sections">
              {SECTIONS.map((item) => (
                <button key={item.id} type="button" onClick={() => selectSection(item.id)} className={`min-h-9 shrink-0 rounded-full px-3 text-[0.75rem]/4 font-semibold ${section === item.id ? "bg-[var(--selection-bg)] text-primary" : "text-tertiary active:bg-elev-3"}`}>
                  {item.label}
                </button>
              ))}
            </nav>
            ...
            <div className="scroll-area min-h-0 flex-1 overflow-y-auto pb-[calc(16px+var(--safe-bottom))]">
              {section === "current" ? (
                <div className="p-4">
                  <div className="rounded-[20px] border border-[var(--border-subtle)] bg-elev-2 p-4">
                    <div className="text-[0.8125rem]/[1.125rem] font-semibold text-tertiary">Current mode</div>
                    <div className="mt-1 text-[1.0625rem]/6 font-semibold text-primary">{activePreset.label}</div>
                    <div className="mt-1 text-[0.75rem]/4 font-medium text-secondary">{activePreset.detail}</div>
                    {activePreset.composite ? <div className="mt-3 text-[0.75rem]/4 font-medium text-tertiary">Specialist deliberation is private. Only Navi’s final response appears in the conversation.</div> : null}
                    <div className="mt-4 border-t border-[var(--border-subtle)] pt-3 text-[0.75rem]/4 font-medium text-tertiary">Available providers: {providerSummary}</div>
                    <div className="mt-1 text-[0.75rem]/4 font-medium text-tertiary">Connector access: {preferences.connectorAccessMode}</div>
                  </div>
                  <button type="button" onClick={() => selectSection("models")} className="mt-3 flex min-h-12 w-full items-center justify-between rounded-2xl px-3 text-left text-[0.9375rem]/[1.375rem] font-medium text-primary active:bg-elev-2">Change Navi mode<ChevronRight size={18} /></button>
                  <button type="button" onClick={() => { onOpenProjects(); onClose(); }} className="flex min-h-12 w-full items-center gap-3 rounded-2xl px-3 text-left text-[0.9375rem]/[1.375rem] font-medium text-primary active:bg-elev-2"><FolderKanban size={18} className="text-accent" /><span className="min-w-0 flex-1">Projects</span><ChevronRight size={18} /></button>
                  <button type="button" onClick={() => selectSection("connections")} className="flex min-h-12 w-full items-center gap-3 rounded-2xl px-3 text-left text-[0.9375rem]/[1.375rem] font-medium text-primary active:bg-elev-2"><Link2 size={18} className="text-accent" /><span className="min-w-0 flex-1">Connectors</span><ChevronRight size={18} /></button>
                  <button type="button" onClick={() => { onOpenHistory(); onClose(); }} className="flex min-h-12 w-full items-center justify-between rounded-2xl px-3 text-left text-[0.9375rem]/[1.375rem] font-medium text-primary active:bg-elev-2">Open conversation history<ChevronRight size={18} /></button>
                </div>
              ) : null}
                ...
                <div className="p-2">
                  {MODEL_PRESETS.map((item) => (
                    <button key={item.id} type="button" onClick={() => selectPreset(item.id)} className={`flex min-h-[62px] w-full items-center gap-3 rounded-2xl px-3 text-left active:bg-elev-3 ${preferences.preset === item.id ? "bg-[var(--selection-bg)]" : ""}`}>
                      <span className="min-w-0 flex-1">
                        <span className="block text-[0.9375rem]/[1.375rem] font-medium text-primary">{item.label}</span>
                        <span className="block text-[0.75rem]/4 font-medium text-tertiary">{item.detail}</span>
                      </span>
                  ...
                  <div className="px-3 pb-2 text-[0.8125rem]/[1.125rem] font-semibold text-secondary">Response style</div>
                  <div className="grid grid-cols-3 gap-1 rounded-2xl bg-elev-2 p-1.5">
                    {RESPONSE_STYLES.map((item) => (
                      <button key={item.id} type="button" onClick={() => selectStyle(item.id)} className={`min-h-10 rounded-xl px-2 text-[0.75rem]/4 font-semibold ${preferences.style === item.id ? "bg-elev-3 text-primary" : "text-tertiary"}`}>{item.label}</button>
                    ))}
                  </div>
                </div>
              ) : null}
              ...
              {section === "tools" ? (
                <div className="divide-y divide-[var(--border-subtle)]">
                  ...
                  <div className="p-4">
                    ...
                    <button type="button" onClick={() => inputRef.current?.click()} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl border border-[var(--border-strong)] bg-elev-2 px-4 text-[0.9375rem]/[1.375rem] font-medium text-primary active:bg-elev-3"><FilePlus2 size={18} />Add files or images</button>
                    {pendingFiles.length ? (
                      <div className="mt-3 rounded-2xl border border-[var(--border-subtle)] bg-elev-2 p-3">
                        <div className="flex items-center justify-between gap-3">
                          <span className="text-[0.8125rem]/[1.125rem] font-semibold text-primary">{pendingFiles.length} attachment{pendingFiles.length === 1 ? "" : "s"} ready</span>
                          <button type="button" onClick={onClearFiles} className="min-h-9 rounded-xl px-2 text-[0.75rem]/4 font-semibold text-danger active:bg-elev-3">Clear</button>
                        </div>
                        <div className="mt-1 line-clamp-2 text-[0.75rem]/4 font-medium text-tertiary">{pendingFiles.map((file) => file.name).join(" · ")}</div>
                      </div>
                    ) : null}
                  </div>
                </div>
              ...
              {section === "connections" ? (
                <div className="p-2">
                  <button type="button" onClick={() => { onOpenConnectors(); onClose(); }} className="mb-2 flex min-h-12 w-full items-center gap-3 rounded-2xl bg-[var(--selection-bg)] px-3 text-left text-[0.875rem]/5 font-semibold text-primary active:bg-elev-3"><Link2 size={18} className="text-accent" /><span className="min-w-0 flex-1">Open connector manager</span><ChevronRight size={18} /></button>
                  {servers.length ? servers.map((server) => {
                    const connected = preferences.connectedMcpServers.includes(server.id);
                    return (
                      <div key={server.id} className="flex min-h-[66px] items-center gap-3 rounded-2xl px-3">
                        <span className="min-w-0 flex-1">
                          <span className="block text-[0.9375rem]/[1.375rem] font-medium text-primary">{server.name}</span>
                          <span className="block truncate text-[0.75rem]/4 font-medium text-tertiary">Remote HTTPS MCP · {server.readOnly ? "read-only" : "writes require confirmation"}</span>
                        </span>
                  ...
                  }) : <div className="px-5 py-10 text-center text-[0.8125rem]/[1.125rem] font-medium text-tertiary">No remote MCP servers are configured in Vercel.</div>}
                  {connectionError ? <div className="mx-3 mt-2 rounded-2xl border border-[var(--accent-danger)] bg-elev-2 p-3 text-[0.75rem]/4 font-medium text-danger">{connectionError}</div> : null}
                </div>
              ) : null}
              ...
              {section === "personalization" ? (
                <div className="divide-y divide-[var(--border-subtle)]">
                  <div className="p-4">
                    <div className="mb-2 text-[0.8125rem]/[1.125rem] font-semibold text-secondary">Theme</div>
                    <div className="grid grid-cols-3 gap-1 rounded-2xl bg-elev-2 p-1.5">
                      {(["dark", "light", "system"] as const).map((theme) => <button key={theme} type="button" onClick={() => update({ theme })} className={`min-h-10 rounded-xl capitalize text-[0.75rem]/4 font-semibold ${preferences.theme === theme ? "bg-elev-3 text-primary" : "text-tertiary"}`}>{theme}</button>)}
                    </div>
                  </div>
                  ...
                  <SettingRow
                    ...
                    action={(
                      <select
                        ...
                        className="min-h-10 rounded-xl border border-[var(--border-subtle)] bg-elev-2 px-2 text-[0.8125rem]/5 font-medium text-primary"
                      >
              ...
              {section === "system" ? (
                <div className="divide-y divide-[var(--border-subtle)]">
                  ...
                  <button type="button" onClick={() => { haptic("selection", preferences.haptics); onExport(); }} className="flex min-h-[58px] w-full items-center gap-3 px-4 text-left active:bg-elev-2">
                    <span className="min-w-0 flex-1">
                      <span className="block text-[0.9375rem]/[1.375rem] font-medium text-primary">Export your data</span>
                      <span className="block text-[0.75rem]/4 font-medium text-tertiary">Download chats, projects, and preferences as JSON</span>
                    </span>
