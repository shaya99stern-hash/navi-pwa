...
export async function generateNaviImage(options: {
  ...
  if (!block) {
    ...
    if (request.mode === "edit") {
      throw new Error(geminiApiKey()
        ? "NaviSol could not edit that image. It will not generate a different picture in its place — try again, or describe the change more specifically."
  ...
  return {
    ...
    engine,
    alt: options.prompt.trim().slice(0, 300) || "Image generated by NaviSol",
