 ...
 * working for everyone — which surfaces as NaviSol quietly getting worse at
 ...
 *    and NaviSol answers from its own knowledge. Silently — a person asking
...
export async function recordSearch(): Promise<void> {
  await getSpendStore().add(counterKey(), 1).catch((error) => {
    console.warn("NaviSol could not record a search against the monthly allowance:", error);
  });
}
