 ...
 */
export async function downscaleImage(file: File, targetBytes: number): Promise<File> {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 6; attempt += 1) {
      ...
      if (attempt >= QUALITY_STEPS.length - 1) scale *= 0.7;
    }
    return file;
  } finally {
    bitmap.close?.();
...
export async function prepareAttachments(files: File[]): Promise<PreparedAttachments> {
  ...
  const resized = await Promise.all(images.map((file) => downscaleImage(file, perImage)));
